...
class DhanClient:
    ...
    def _load_ingestion_config(self):
    ...
    def get_daily_spot_data(
        ...
        if under_security in ["NIFTY", "BANKNIFTY"]:
            exchange_segment = "IDX_I"
            instrument_type = "INDEX"
    ...
    def get_intrday_spot_data(
        ...
        """
        Fetch intraday data iteratively using
        Dhan REST API direct payload calls.

        Dhan allows maximum 90-day intraday fetch per request.
        This function reconstructs long-range intraday memory
        through iterative batching.
        """
        ...
        total_lookback_days = config["lookback_days"]
        ...
        final_start_date = (
                final_end_date -
                timedelta(days=total_lookback_days)
        )

        logger.info(
            f"Fetching {total_lookback_days} days "
            f"of intraday data for {under_security}"
        )
        ...
        while current_end_date > final_start_date:
            ...
            payload = {
                ...
                "fromDate": current_start_date.strftime(
                    "%Y-%m-%d"
                ),

                "toDate": current_end_date.strftime(
                    "%Y-%m-%d"
                ),
        ...
        if len(all_frames) == 0:
            logger.warning(
                f"No intraday data collected "
                f"for {under_security}"
            )
        ...
        logger.info(
            f"Final dataframe rows :: "
            f"{len(final_df)}"
        )
        # To rename in-place without re-assigning:
