...
import typer
def universe(
    csv_file: str = "config/instruments/nifty50.csv"
):
    """
...
if __name__ == "__main__":
    ...
    generate_daily_dataframe_report(june_df_filtered, 'query_op/DAILY_REPORTS/nifty50_daily_report.pdf')
    june_df_filtered.to_sql(f'nifty50_monthly_{today_str}', conn, if_exists="replace", index=False)
    june_df_filtered.to_csv(f'query_op/filtered/nifty50_monthly_{today_str}.csv', index=False)
    q2 = recall.recall_quarter(2026, 2)
    q2_df = MemoryLogger.to_dataframe(q2)
    q2_df_filtered = filter_universe(q2_df, ltp_df)
    q2_df_filtered.to_csv(f'query_op/filtered/nifty50_quarterly_{today_str}.csv', index=False)
    q2_df_filtered.to_sql(f'nifty50_quarterly_{today_str}', conn, if_exists="replace", index=False)